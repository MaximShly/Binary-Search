...
const searchForAlphabeticalIndex = ([stringToFindIndexForFirstLetter], alphabetArray = generateAlphabetArray()) => {
    ...
    let recursiveBinarySearch = (array, letter, startIndex, endIndex) => {
        ...
        // }
    // base case for not finding the letter in the given array
    if (startIndex > endIndex) {
        binarySearchResults = -1;
        return false;
    }

    // calculate middle index between the given start and end
    let midIndex = Math.floor((startIndex + endIndex) / 2);

    // check if the value at midIndex is equal to the character being searched for
    ...
    if (letter < array[midIndex]) {
        // if the letter is less than the midIndex element, search the lower (start to middle) portion of the dataset
        return recursiveBinarySearch(array, letter, startIndex, midIndex - 1);
    } else {
        // if the letter is greater than the midIndex element, search the upper (middle to end) portion of the dataset
        return recursiveBinarySearch(array, letter, midIndex + 1, endIndex);
    }
